...
//  SwiftScanner1
//
//  Created by Tim Skapek on 10/22/19.
...
class FirstScanViewController: UIViewController, STBackgroundTaskDelegate, MeshViewDelegate, UIGestureRecognizerDelegate, AVCaptureVideoDataOutputSampleBufferDelegate {
...
//    @IBOutlet weak var calibrationOverlay: UIView!
    ...
    @IBOutlet weak var mEview: EAGLView!
    
    @IBOutlet weak var mEnableNewTrackerSwitch: UISwitch!
    @IBOutlet weak var mEnableHighResolutionColorSwitch: UISwitch!
    @IBOutlet weak var mEnableNewMapperSwitch: UISwitch!
    @IBOutlet weak var mEnableHighResMappingSwitch: UISwitch!
    
    @IBOutlet weak var mScanButton: UIButton!
    @IBOutlet weak var mAppStatusMessageLabel: UILabel!
    @IBOutlet weak var mResetButton: UIButton!
    @IBOutlet weak var mDoneButton: UIButton!
    
    @IBOutlet weak var mTrackingLostLabel: UILabel!
    @IBOutlet weak var mEnableNewTrackerView: UIStackView!
    @IBOutlet weak var mInstructionOverlay: UIStackView!
    @IBOutlet weak var mCalibrationOverlay: UIStackView!
    ...
    override func viewDidLoad() {
        ...
        mCalibrationOverlay.alpha = 0
        mCalibrationOverlay.isHidden = true
        mCalibrationOverlay.isUserInteractionEnabled = false
        
        mInstructionOverlay.alpha = 0
        mInstructionOverlay.isHidden = true
        mInstructionOverlay.isUserInteractionEnabled = false
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if !defaults.bool(forKey: "instructionOverlay") {
            ...
            let _ = Timer.schedule(10.0, handler: {_ in
                self.mInstructionOverlay.isHidden = false
                self.mInstructionOverlay.isUserInteractionEnabled = true
                self.mInstructionOverlay.alpha = 1
                let _ = Timer.schedule(15.0, handler: { _ in
                    UIView.animate(withDuration: 0.3, animations: {
                        
                        self.mInstructionOverlay.alpha = 0
                        self.mInstructionOverlay.isHidden = true
                        self.mInstructionOverlay.isUserInteractionEnabled = false
                    })
                })
            })
        }
    ...
    func syncUIfromDynamicOptions() {
        ...
        mEnableHighResMappingSwitch.isEnabled = _dynamicOptions.highResMappingSwitchEnabled
        
        mEnableNewMapperSwitch.isOn = _dynamicOptions.newMapperIsOn
        mEnableNewMapperSwitch.isEnabled = _dynamicOptions.newMapperSwitchEnabled
        
        mEnableHighResolutionColorSwitch.isOn = _dynamicOptions.highResColoring
        mEnableHighResolutionColorSwitch.isEnabled = _dynamicOptions.highResColoringSwitchEnabled
    ...
    func setupUserInterface() {
        ...
        mAppStatusMessageLabel.alpha = 0
    }
    ...
    func enterCubePlacementState() {
        ...
        mTrackingLostLabel.isHidden = true
    ...
    func enterScanningState() {
        ...
        mEnableNewTrackerSwitch.isEnabled = false
        mEnableHighResolutionColorSwitch.isEnabled = false
        mEnableNewMapperSwitch.isEnabled = false
        mEnableHighResMappingSwitch.isEnabled = false
    }
    ...
    @IBAction func newTrackerSwitchChanged(sender: UISwitch) {
        ...
        _dynamicOptions.newTrackerIsOn = mEnableNewTrackerSwitch.isOn
    ...
    @IBAction func highResolutionColorSwitchChanged(sender: UISwitch) {
        ...
        _dynamicOptions.highResColoring = self.mEnableHighResolutionColorSwitch.isOn
    ...
    @IBAction func newMapperSwitchChanged(sender: UISwitch) {
        ...
        _dynamicOptions.newMapperIsOn = self.mEnableNewMapperSwitch.isOn
        onSLAMOptionsChanged() // will call UI sync
    }
    
    @IBAction func highResMappingSwitchChanged(sender: UISwitch) {
        
        _dynamicOptions.highResMapping = self.mEnableHighResMappingSwitch.isOn
        onSLAMOptionsChanged() // will call UI sync
    }
    ...
    @IBAction func mScanButtonPressed(_ sender: Any) {
            // hide windows while scanning
                trackerShowingScanStart =  !mEnableNewTrackerView.isHidden
...
//    }
    ...
    @IBAction func mResetButtonPressed(_ sender: UIButton) {
        // restore window after scanning
        if trackerShowingScanStart {
            toggleTracker(show: true)
        }
...
//    }
    ...
    @IBAction func mDoneButtonPressed(_ sender: UIButton) {
        // restore window after scanning
        if trackerShowingScanStart{
            toggleTracker(show: true)
        }
    ...
    func showTrackingMessage(_ message: String) {
        ...
        mTrackingLostLabel.text = message
        mTrackingLostLabel.isHidden = false
    }
    ...
    func showAppStatusMessage(_ msg: String) {
        ...
        mAppStatusMessageLabel.text = msg
        mAppStatusMessageLabel.isHidden = false
        
        // Progressively show the message label.
        mEview!.isUserInteractionEnabled = false
        UIView.animate(withDuration: 0.5, animations: {
            self.mAppStatusMessageLabel.alpha = 1.0
        })
    }
    ...
    func hideAppStatusMessage() {
        ...
        UIView.animate(withDuration: 0.5, animations: {
            self.mAppStatusMessageLabel.alpha = 0
        }, completion: { _ in
            ...
            if !self._appStatus.needsDisplayOfStatusMessage {
                ...
                // Could be nil if the self is released before the callback happens.
                if self.mEview != nil {
                    self.mAppStatusMessageLabel.isHidden = true
                    self.mEview.isUserInteractionEnabled = true
                }
            }
        })
    }
    ...
    @IBAction func toggleNewTrackerVisible(sender: UILongPressGestureRecognizer) {
        ...
        if (sender.state == .began) {
            ...
            toggleTracker(show: mEnableNewTrackerView.isHidden)
        }
    }
    ...
    func toggleTracker(show: Bool) {
        ...
        if show {
            // set alpha to 0.9
            mEnableNewTrackerView.alpha = 0
            mEnableNewTrackerView.isHidden = false
            UIView.animate(withDuration: 0.3, delay: 0.0, options: .curveEaseOut, animations: { () -> Void in
                self.mEnableNewTrackerView.alpha = 0.9
            }, completion: { (finished: Bool) -> Void in
                self.mEnableNewTrackerView.isHidden = false
            })
        } else {
            // set alpha to 0.0
            UIView.animate(withDuration: 1.0, delay: 0.0, options: .curveEaseOut, animations: { () -> Void in
                self.mEnableNewTrackerView.alpha = 0.0
                
            }, completion: { (finished: Bool) -> Void in
                self.mEnableNewTrackerView.isHidden = true
            })
        }
    }
