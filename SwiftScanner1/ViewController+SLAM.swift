...
extension FirstScanViewController {
    ...
    func processDepthFrame(_ depthFrame: STDepthFrame, colorFrame: STColorFrame?) {
        ...
        case .cubePlacement:
            ...
            self.mScanButton.isEnabled = _slamState.cameraPoseInitializer!.lastOutput.hasValidPose.boolValue
            
            if self.mScanButton.isEnabled {
                self.mScanButton.isUserInteractionEnabled = true
            }
            else {
                self.mScanButton.isUserInteractionEnabled = false
            }
...
extension FirstScanViewController: STSensorControllerDelegate {
    ...
    func sensorDidConnect() {
        ...
        if !mCalibrationOverlay.isHidden {
            mCalibrationOverlay.alpha = 1
            mCalibrationOverlay.isUserInteractionEnabled = true
        }
    }
    ...
    func sensorDidDisconnect() {
        ...
        if _slamState.scannerState == .scanning {
            mResetButtonPressed(mScanButton)
        }
...
//         calibration and the app knows how to calibrate this iOS device.
        if calibrationType != .deviceSpecific && deviceIsLikelySupportedByCalibratorApp {
            ...
            mCalibrationOverlay.alpha = 1
            mCalibrationOverlay.isHidden = false
            mCalibrationOverlay.isUserInteractionEnabled = true
        }
        else {
            mCalibrationOverlay.alpha = 0
